...
(define-module (conner packages)
  ...
  #:replace (%patch-path
             search-patch)
  #:export (conner-patches
            %conner-package-module-path))
...
(define %conner-root-directory
  ...
  (letrec-syntax ((dirname* (syntax-rules ()
                  ...
                  (try      (syntax-rules ()
                              ...
                              ((_)
                               #f))))
    (try ("conner/packages/rust.scm" conner/ packages/)
         ("conner/users.scm" conner/))))

(define %conner-package-module-path
  `((,%conner-root-directory . "conner/packages")))
...
(define %patch-path
  ...
  (make-parameter
   (map (lambda (directory)
          (if (string=? directory %conner-root-directory)
              (string-append directory "/conner/packages/patches")
              directory))
        %load-path)))
...
(define-syntax-rule (conner-patches file-name ...)
