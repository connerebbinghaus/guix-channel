...
(define-public %conner-packages
  (list
   file
   git direnv ripgrep
   emacs guile-3.0 emacs-guix emacs-geiser emacs-geiser-guile emacs-paredit emacs-magit emacs-envrc emacs-org emacs-lsp-mode emacs-lsp-ui emacs-dap-mode emacs-flycheck emacs-company
...
eval \"$(direnv hook bash)\""))))))
	   ...
	   (simple-service 'my-env-vars home-environment-variables-service-type
			   `(("XDG_DATA_DIRS" . "$XDG_DATA_DIRS:$XDG_DATA_HOME/flatpak/exports/share/")
			     ...
			     ("PRISMLAUNCHER_JAVA_PATHS" . ,#~(string-join (map (lambda (s) (string-append s "/bin/java")) (list #$icedtea-8 #$openjdk11 #$openjdk16 #$openjdk17 #$openjdk21 #$openjdk)) ":"))))
	   %base-home-services))))
